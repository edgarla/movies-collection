...
import infrastructure.util.model.DatabaseSequence;
...
public class SequenceServiceImpl implements SequenceService {
  ...
  @Override
  public UUID getNextSequenceId(String sequenceName) {
    DatabaseSequence sequence = mongoOperations.findAndModify(
            query(where("_id").is(sequenceName)),
            new Update().inc("seq",1),
            options().returnNew(true).upsert(true),
            DatabaseSequence.class);
    return !Objects.isNull(sequence) ? sequence.sequence() : UUID.randomUUID();
  }
}
